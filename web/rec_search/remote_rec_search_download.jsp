...
<%@ page
		import="com.cnet.crec.common.Mask"
%>
...
<%
	...
	try 
	{
		...
		String download_name = CommonUtil.getParameter("download_name");

		if (rec_mode.equals("A")) rec_mode = "";
		...
		*/
		...
		argMap.put("download_name", download_name);
		// 사용자 권한에 따른 녹취이력 조회
		...
		// record count
		Map<String, Object> cntmap = db.selectOne("rec_search_download.selectCount", argMap);
		...
		*/
		...
		List<Map<String, Object>> list = db.selectList("rec_search_download.selectList", parMap);
		int row_indx = 0;
		/*
		String custTel = "";
		String vCustTel = "";
		...
		for(Map<String, Object> item : list) 
		{
			...
			if(item.containsKey("n_rec_keycode") && item.get("n_rec_keycode") != null){
				...
				if (_LOGIN_LEVEL.equals("E"))
					...
					item.put("n_rec_keycode", "<img class='copy' src='../img/icon/ico_memo.png' data-clipboard-text='" + item.get("n_rec_keycode").toString().trim() + "' style='margin-left: 5px; cursor: pointer;'/> " + temp);
			}
			if(item.containsKey("n_rec_filename") && item.get("n_rec_filename") != null){
				String temp =  item.get("n_rec_filename").toString().trim();
//				if (_LOGIN_LEVEL.equals("E"))
//					item.put("n_rec_keycode", temp);
//				else
